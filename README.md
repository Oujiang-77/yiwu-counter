...
本机 API + SQLite + 普通浏览器界面，内置 RapidOCR / ONNX Runtime CPU 离线文字识别。目标 Windows 10/11 x64；首个本地试用版本 0.1.1，无在线账号、云数据库或付费接口。
...
构建脚本生成 `dist/档口开单系统/档口开单系统.exe` 与上一级 `outputs/档口开单系统-0.1.1-Windows-x64.zip`。最终只打包冻结程序、使用说明和依赖许可，不含测试数据库、用户照片、厂家资料或开发环境。依赖版本已锁定；首次构建需要联网安装依赖，用户运行不需要。

## 当前边界

- 尚未收到实际厂家 Excel 或指定报货模板，当前为可映射字段的通用导入与报货单。
- 离线文字识别不能保证手写订单准确。所有识别行须人工核对，无法匹配时手动选择，不能确定的单位不猜测。
- 用户已确认先交付本地试用包。源码私有仓库为 `Oujiang-77/yiwu-counter`；本次基础包已内置 GitHub 更新检查和自动更新助手。后续发布新版本后，在“检查更新”中点击“立即更新并重启”即可，更新助手只替换程序目录，不会覆盖数据目录。
