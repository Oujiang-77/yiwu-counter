...
本机 API + SQLite + 普通浏览器界面，内置 RapidOCR / ONNX Runtime CPU 离线文字识别。目标 Windows 10/11 x64；当前版本 0.1.2，无在线账号、云数据库或付费接口。
...
构建脚本生成 `dist/档口开单系统/档口开单系统.exe` 与上一级 `outputs/档口开单系统-0.1.2-Windows-x64.zip`。最终只打包冻结程序、使用说明和依赖许可，不含测试数据库、用户照片、厂家资料或开发环境。依赖版本已锁定；首次构建需要联网安装依赖，用户运行不需要。
