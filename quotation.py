...
EXPORT_FIELDS={'name':'商品名称','images':'主图','boxImages':'彩盒','code':'销售编码','factoryCode':'厂家编码','factory':'厂家名称','size':'尺寸','battery':'电池容量','charger':'充电头 / 接口','pack':'装箱数','quantity':'件数','units':'总数量','price':'出厂价','amount':'金额','volume':'单件体积','totalVolume':'总体积','cat':'分类','notes':'备注'}

def validate_columns(columns):
    if not isinstance(columns,list) or not columns or any(not isinstance(k,str) or k not in EXPORT_FIELDS for k in columns):raise ValueError('请至少选择一个有效的导出字段')
...
def quotation_bytes(items,store,columns=None):
    ...
    for index,(factory,products) in enumerate(groups.items(),1):
        ...
        ws.append(['说明','1 件 = 1 箱；金额按出厂单价计算，不含运费。'])
        ...
        for row,p in enumerate(products,4):
            ...
            for col,key in enumerate(columns,1):
                if key in ('images','boxImages'):
                    ...
                    continue
                value=p.get(key,'')
                if key in ('amount','totalVolume'):value=float(value)
                cell=ws.cell(row,col,value)
                if isinstance(value,str):cell.data_type='s'
                cell.alignment=Alignment(vertical='center',wrap_text=True)
                if key in ('price','amount'):cell.number_format='0.00'
                if key in ('volume','totalVolume'):cell.number_format='0.000000'
            ws.row_dimensions[row].height=height
        last=3+len(products);total=last+1
        label_col=next((i for i,k in enumerate(columns,1) if k not in ('quantity','units','amount','totalVolume')),None)
        ...
        for col,key in enumerate(columns,1):
            ...
            if key in ('quantity','units','amount','totalVolume'):
                ws.cell(total,col,f'=SUM({letter}4:{letter}{last})')
                ws.cell(total,col).number_format='0.00' if key=='amount' else '0.000000' if key=='totalVolume' else '0'
