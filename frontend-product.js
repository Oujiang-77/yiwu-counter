...
function newForm(id){
 ...
 const fields=[['code','销售编码',true],['factoryCode','厂家编码',true],['name','商品名称',true],['factory','厂家名称',false],['color','颜色',false],['size','尺寸',false],['cartonSize','外箱尺寸',false],['cartonWeight','单箱重量（kg）',false],['material','材质',false],['pack','装箱数（个 / 件）',false],['price','出厂价（元 / 个）',false],['battery','电池容量',false],['charger','充电头 / 接口',false],['volume','单件体积（m³）',false],['cat','商品分类',false]];
 openDialog(id?'编辑商品资料':'新增商品','保存后写入本机数据库。','<form id="productForm" data-id="'+(id||'')+'"><div id="productMedia" class="product-media">'+Object.keys(mediaLabels).map(mediaGroup).join('')+'</div><div class="form-grid">'+fields.map(([k,l,required])=>'<label><span class="form-label">'+l+(required?' <em class="required-mark">*</em>':'')+'</span><input name="'+k+'" value="'+esc(p[k]??'')+'" '+(k==='pack'?'type="number" min="0" max="1000000" step="1"':k==='price'?'type="number" min="0" max="100000000" step="0.01"':k==='volume'?'type="number" min="0" max="100000000" step="0.000001"':k==='cartonWeight'?'type="number" min="0" max="100000" step="0.001"':'maxlength="300"')+(required?' required':'')+'></label>').join('')+'<label style="grid-column:1/-1"><span class="form-label">产品参数</span><textarea name="parameters" maxlength="3000">'+esc(p.parameters||'')+'</textarea></label><label style="grid-column:1/-1"><span class="form-label">备注</span><textarea name="notes" maxlength="300">'+esc(p.notes||'')+'</textarea></label></div></form>','<small>销售编码唯一；厂家编码在同一厂家内唯一。</small><button class="primary" type="submit" form="productForm">保存商品</button>');
}
